  ...
  async run(msg: commando.CommandMessage): Promise<Message> {
    ...
    request
      .get(`http://localhost:8000/users/${msg.author.id}`, function optionalCallback(err, httpResponse, body) {
        if (body === '[]') {
          return msg.reply(`It seems as if I didn't have you in the database. Please try again.`)
        }
        user = JSON.parse(body)
        if (user.balance.dateclaimeddailies) {
          let diffHrs = Math.abs(new Date().getTime() - parseInt(user.balance.dateclaimeddailies)) / 36e5
          if (diffHrs >= 24) {
            ...
            msg.channel.send(`**${user.name}**, you still have **${waitTime}** hours until you can claim your dailies again.`)
          }
        } else {
          user.balance.balance = `${parseInt(user.balance.balance) + 200}`
          user.balance.networth = `${parseInt(user.balance.networth) + 200}`
